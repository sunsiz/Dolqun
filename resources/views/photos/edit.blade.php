                    ...
                    <div class="panel-body">
                        ...
                        <form action="{{ route('photos.update', $photo->id) }}" method="post">
                            ...
                            <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                                <input type="text" class="form-control"  placeholder="ماۋزۇ (چوقۇم يازىسىز)" name="title" value="{{ $photo->title }}">
                                @if ($errors->has('title'))
                            ...
                            <div class="form-group">
                                <label class="radio-inline">
                                    <input type="radio" id="inlineCheckbox1" value="1" name="type" @if($photo->type==1) checked @endif>  تەگلىك
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" id="inlineCheckbox2" value="2" name="type" @if($photo->type==2) checked @endif> سىزما
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" id="inlineCheckbox3" value="3" name="type" @if($photo->type==3) checked @endif> كوسپىلاي
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" id="inlineCheckbox4" value="4" name="type" @if($photo->type==4) checked @endif>باش سۈرەت
                                </label>
                            </div>

                            <div class="form-group">
                                <photo avatar="{{ $photo->thumb }}"></photo>
                            </div>

                            <div class="form-group {{ $errors->has('body') ? 'has-error' : '' }}">
                                <textarea id="container" name="body" type="text/plain"> {{ $photo->body }}</textarea>
                                @if ($errors->has('body'))
                                    <span class="text-danger">
